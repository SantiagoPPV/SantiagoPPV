...
const getHarvestDaysColor = (days: number | undefined, sector: string): string => {
  if (typeof days !== 'number') return '#6B7280'; // gris si no hay datos

  const isEarlySector = /^[12][A-E]$/.test(sector.toUpperCase());

  if (isEarlySector) {
    if (days < 10) return '#22c55e';      // verde
    if (days <= 12) return '#eab308';     // amarillo
    return '#ef4444';                     // rojo
  } else {
...
function getIrrigationColor(caudal: number, sector: string): string {
  ...
  if (isBiloxi) {
    if (caudal >= 14 && caudal <= 16) return '#22c55e'; // verde
    if ((caudal >= 12 && caudal < 14) || (caudal > 16 && caudal <= 18)) return '#eab308'; // amarillo
    return '#ef4444'; // rojo
  } else {
    if (caudal >= 19 && caudal <= 21) return '#22c55e'; // verde
    if ((caudal >= 17 && caudal < 19) || (caudal > 21 && caudal <= 23)) return '#eab308'; // amarillo
    return '#ef4444'; // rojo
  }
}
  ...
  for (const key in grouped) {
    ...
    metrics[key] = {
      ...
      const { data, error } = await supabase
        ...
        .select('Sector, Tuneles, "Dias a cosecha"')
        .eq('Semana', parseInt(selectedWeek));

      if (error) throw error;

      data?.forEach(record => {
        const tunnels = record.Tuneles?.split(',').map(t => t.trim()) || [];
        tunnels.forEach(tunnel => {
          const key = `${record.Sector} Tunel ${tunnel}`;
          metrics[key] = {
            harvestDays: record['Dias a cosecha'],
            details: record['Dias a cosecha'] !== undefined
              ? `Días desde última cosecha: ${record['Dias a cosecha']}`
              : 'Sin datos de cosecha para este túnel'
          };
        });
      });
  ...
  // Otros tipos de muestreo conservan su comportamiento actual
   switch (samplingType) {
    case 'pests':
      ...
      return /^[1-7][A-E]\s+TUNEL\s+\d+$/i.test(name);
    case 'nutrients':
    case 'harvest':
    case 'irrigation':
      return isPolygon && !isSector;
    case 'Pressures':
      return isPoint;
    default:
      return true;
...
//     <label className="block text-sm font-medium mb-2">Fecha</label>
//     <select
//       value={selectedNutrientDate}
//       onChange={(e) => setSelectedNutrientDate(e.target.value)}
//       className="w-full bg-[#2A2A2A] border border-[#404040] rounded-lg px-4 py-2 text-white"
//       disabled={availableNutrientDates.length === 0}
//     >
//       {availableNutrientDates.length === 0 ? (
//         <option value="">No hay fechas disponibles</option>
//       ) : (
//         availableNutrientDates.map(date => (
//           <option key={date} value={date}>
//             {new Date(date + 'T06:00:00Z').toLocaleDateString('es-MX', {
//               weekday: 'long',
//               day: 'numeric',
