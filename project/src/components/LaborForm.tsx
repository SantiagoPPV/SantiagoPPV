...
>({});
  ...
  secciones.forEach(s => {
    const match = s.match(/^([1-7][A-E])\s+Tunel\s+\d+$/);
    if (match) {
 ...
 useEffect(() => {
  const fetchAllData = async () => {
    const [laboresResp, muestreoResp] = await Promise.all([
      supabase.from('Labores Personal').select('*').eq('Activo', true),
      supabase.from('Muestreos Labores').select('Seccion, "Nombre Labor"')
    ]);
    ...
    const laborData = laboresResp.data.filter(l => l.Tipo?.toUpperCase().trim() !== 'N/A');
    ...
    const todasSecciones = laborData
      ...
      .map(s => s.trim())
      .filter(s => /^([1-7][A-E])\s+Tunel\s+\d+$/.test(s));
  ...
  const fetchSectoresCompletos = async () => {
    ...
    data?.forEach(row => {
      (row.Seccion || '')
        ...
        .map(s => s.trim())
        .forEach(sec => {
          const match = sec.match(/^([1-7][A-E])\s+Tunel\s+(\d+)$/);
          if (match) {
