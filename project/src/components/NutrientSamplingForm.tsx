  ...
  const { error } = await supabase
    ...
    .insert(
      entries.map(entry => ({
        ...
        'C.E.': parseFloat(parseFloat(entry.ce).toFixed(2)),  // hasta 2 decimales
        pH: parseFloat(parseFloat(entry.ph).toFixed(1)),      // hasta 1 decimal
      ...
      <div className="bg-[#1A1A1A] p-6 rounded-lg">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select
              value={entries[0].sector}
              onChange={(e) => handleEntryChange('1', 'sector', e.target.value)}
              className={getInputClassName(errors.entries['1']?.sector || false)}
            >
